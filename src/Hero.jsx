export default function Hero() {
  return (
    <section className="bg-black text-white min-h-[90vh] px-2 flex flex-col justify-between py-24 lg:px-24">
      {/* Espace central vide */}
      ...
      <div className="flex justify-between items-end md:flex-row flex-wrap">
        ...
        {/* Bas droite — stats */}
        <div className="bg-[#0d0d1a] border border-gray-800 rounded-2xl px-6 py-6 flex gap-4">
          {[
            { value: "5+", label: "Expériences" },
            { value: "20+", label: "Projets réalisés" },
            { value: "80+", label: "Clients contents" },
          ].map((stat, i) => (
            <div key={i} className={`text-center ${i < 2 ? "" : ""}`}>
